...
def main():
    ...
    # their weights. Normalization now lives in an external processor pipeline
    # built from the dataset stats, and must be applied to every batch before
    # policy.forward(). See ACT_MODIFICATIONS.md.
    preprocessor, postprocessor = make_pre_post_processors(
        policy_cfg=cfg,
        dataset_stats=schema_meta.stats,
    )
    ...
    while not done:
        for batch in dataloader:
            ...
            # expects float in [0, 1], then the preprocessor applies the
            # mean/std normalization that used to live inside the policy.
    ...
    # CHANGED (lerobot v0.6.0): the normalization stats now live in the processor
    # pipelines rather than in the model weights, so they must be saved next to
    # the policy. Without these the checkpoint cannot be run at rollout time.
