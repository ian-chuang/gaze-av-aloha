import numpy as np
import time
from gaze_av_aloha.robot.config import REAL_DT
...
def sleep_2arms(puppet_bot_left, puppet_bot_middle):
    all_bots = [puppet_bot_left,  puppet_bot_middle]
    for bot in all_bots:
        torque_on(bot)
    ...
    middle_puppet_sleep_position = (0, -1.7, 1.55, 0, 0.65, 0, 0)
    move_arms(all_bots, [left_puppet_sleep_position,middle_puppet_sleep_position], move_time=3)
...
def sleep_no_left(puppet_bot_right, puppet_bot_middle):
    all_bots = [puppet_bot_right, puppet_bot_middle]
    for bot in all_bots:
        torque_on(bot)
