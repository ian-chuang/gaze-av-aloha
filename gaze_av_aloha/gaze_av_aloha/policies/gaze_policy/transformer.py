...
class CrossAttentionBlock(nn.Module):
    """
    Cross-attention block that uses DiT as the backbone.
    """
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        super().__init__()
    ...
    def forward(self, x, context):
        ...
        mask: Optional attention mask
        """
        x = x + self.xattn(self.norm1(x), context)
...
class AttentionPooling(nn.Module):
    ...
    """
    def __init__(self, hidden_size, out_dim, num_queries, depth=2, num_heads=8, mlp_ratio=4.0, dropout=0.0, max_seq_len=64):
        super().__init__()
