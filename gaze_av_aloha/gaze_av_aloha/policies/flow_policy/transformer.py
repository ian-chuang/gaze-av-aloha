...
class AttentionBlock(nn.Module):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, dropout=0.0):
        ...
        self.xattn = nn.MultiheadAttention(hidden_size, num_heads, dropout=dropout, batch_first=True)
        self.norm3 = nn.LayerNorm(hidden_size)
        self.mlp = Mlp(in_features=hidden_size, hidden_features=int(hidden_size * mlp_ratio), drop=dropout)

    def forward(self, x, c, c_pos_emb=None):
        """
        x: [B, S, D] - Query input
        context: [B, S_ctx, D] - Key/Value input
        mask: Optional attention mask
        ...
        q = self.norm2(x)  
        k = c + c_pos_emb if c_pos_emb is not None else c
        v = c
        x = x + self.xattn(query=q, key=k, value=v, need_weights=False)[0]
