        ...
        if any(name.startswith(prefix) for prefix in startswith):
            if name in src_state and src_state[name].shape == param.shape:
                ...
                if not torch.equal(param.data, src_state[name].data):
                    print(
                        f"Weight mismatch for {name}: source and target data are not equal."
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint", type=str, default="", help="Path to the checkpoint file"
    )
    ...
    check_weights(
        model.encoder, vit, startswith=["patch_emb", "pos_enc", "reg_tokens", "blocks"]
    )

    copy_matching_weights(
        vit, model.encoder, startswith=["patch_emb", "pos_enc", "reg_tokens", "blocks"]
    )
