...
def main_worker(rank, world_size, args):
    ...
    print(
        f"[Rank {rank}] Using CUDA device: {torch.cuda.current_device()} / real GPU: {os.environ.get('CUDA_VISIBLE_DEVICES')}"
    )
    ...
    if os.path.exists(model_path):
        ...
        start_epoch = load_checkpoint(
            model_path, model, optim, lr_scheduler, device=device
        )
    ...
    for e in range(start_epoch, args.total_epoch):
        ...
        if rank == 0:
            ...
            with torch.no_grad():
                ...
                viz_pred_tokens[~mask] = tokens[
                    ~mask
                ]  # Fill masked tokens with original tokens
...
def main():
    ...
    parser.add_argument(
        "--add_noise", action="store_true", help="Add noise to the input images"
    )
    parser.add_argument(
