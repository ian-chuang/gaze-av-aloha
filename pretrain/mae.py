...
from gaze_av_aloha.policies.gaze_policy.vit import (
    named_apply,
    init_weights_vit_timm,
    extend_valid_token_mask,
    get_activation_fn,
)
...
class MAE_Encoder(torch.nn.Module):
    ...
    def forward(
        ...
        """
        # normal
        tokens = self.patch_emb(tokens.flatten(2)) + self.pos_enc.unsqueeze(
            0
        )  # (B, S, D)
        ...
        tokens, mask, forward_indexes, backward_indexes = self.shuffle.forward(
            tokens, mask
        )
...
class MAE_Decoder(torch.nn.Module):
    def __init__(
        ...
        self.pos_embedding = torch.nn.Parameter(
            torch.zeros(num_tokens + num_registers, 1, embedding_dim)
        )
        self.num_registers = num_registers
        self.embed_dim = embedding_dim

        self.transformer = torch.nn.Sequential(
            *[Block(embedding_dim, num_heads) for _ in range(depth)]
        )
    ...
    def forward(self, features, backward_indexes):
        ...
        backward_indexes = torch.cat(
            [
                ...
                backward_indexes + self.num_registers,
            ],
            dim=0,
        )
        features = torch.cat(
        ...
        mask = take_indexes(
            mask, backward_indexes[self.num_registers :] - self.num_registers
        )
